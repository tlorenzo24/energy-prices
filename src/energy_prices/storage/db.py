...
def _enable_timescale(engine: Engine) -> None:
    """Best-effort TimescaleDB setup on Postgres. No-op / warning if unavailable."""
    hypertables = {
    ...
    with engine.begin() as conn:
        ...
        for table, time_col in hypertables.items():
            try:
                ...
                logger.warning("Could not create hypertable %s: %s", table, exc)
